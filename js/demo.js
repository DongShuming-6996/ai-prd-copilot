...
(function (global) {
  ...
  var SAMPLE_CONTENT = {
    ...
    improvements:
      "| 优先级 | 现状 | 改后 | Deadline |\n| --- | --- | --- | --- |\n| P0 | 列表页无质检结果信息 | 新增\"质检结果\"列（自动结果 + 人工状态） | 待填写 |\n| P0 | 仅基础筛选 | 新增质检结果筛选器，支持多选 | 待填写 |\n| P1 | 命中问题无视觉区分 | 整行高亮 + 结果列红色标签 | 待填写 |\n| P1 | 仅按时间排序 | 支持按结果状态、问题等级排序 | 待填写 |\n| P2 | 列表列固定写死 | 列自定义（二期） | 二期 |\n\n**动作优先级说明**：先结果列（P0）→ 筛选（P0）→ 高亮与排序（P1）→ 列自定义（P2）。Deadline 由用户填写。",
    data:
    ...
    function fill(def) {
      switch (def.key) {
        ...
        case "improvements": {
          ...
          return bs.map(function (b) { return "- [P0] " + b + "（Deadline：待填写）"; }).join("\n");
        }
        case "data":
          return crossDept
  ...
  function genericQuestions(sections, materials, prefs) {
    ...
    sections.forEach(function (sec) {
      if (/待补充|待填写|未命名/.test(sec.content)) {
        push(1, sec.key, "原始材料未覆盖「" + sec.title + "」，请补充。", "", "P0", false);
      }
    });
    ...
    if (improvements && prefs.askDeadline && /待填写/.test(improvements.content)) {
      push(2, "improvements", "各优先级对应的 Deadline 是什么？（AI 不代排）", "用户填写", "P0", false);
    }
    if (dataSec && prefs.askDataSource && /待补充/.test(dataSec.content)) {
      push(2, "data", "相关数据（表 / 字段）的数据来源是什么？如何关联进入系统？", "待确认", "P0", true);
    }
    if (dataSec && prefs.askDataSource) {
      push(2, "data", "涉及字段的计算逻辑与最小单位是否需要补充？", "按需确认", "P1", true);
    }
    if (acceptance && /待补充/.test(acceptance.content)) {
      push(2, "acceptance", "验收标准是否有可量化指标（性能 / 时间 / 成功率）？", "待确认", "P1", false);
    }
    if (text.indexOf("权限") >= 0 || text.indexOf("角色") >= 0) {
      var sec2 = sections.find(function (s) { return s.key === "backend"; }) || sections[0];
      push(2, sec2.key, "不同角色的权限边界如何定义？可见范围是什么？", "待确认", "P1", false);
    }
    if (/建议|大概|可能|约/.test(text)) {
      push(2, "background", "材料中出现模糊表述（建议 / 大概 / 约），请确认实际口径。", "待确认", "P1", false);
    }
    if (nameSec && /未命名/.test(nameSec.content)) {
      push(1, "name", "项目名称定为什么？", "待命名", "P0", false);
    }
    return out;
  }
