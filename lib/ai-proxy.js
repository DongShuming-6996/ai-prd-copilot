...
// 用 Node 内置 http/https 模块发请求（与 curl 同路径，规避 undici fetch 的连接问题）
        ...
        res.on("end", () => {
          ...
          if (res.statusCode && res.statusCode >= 200 && res.statusCode < 300) {
            try {
              ...
              reject(new Error("模型响应不是合法 JSON"));
            }
          } else {
            reject(new Error(`模型服务错误 ${res.statusCode}：${text.slice(0, 300)}`));
          }
        });
      }
    );
    req.setTimeout(timeoutMs || 120000, () => req.destroy(new Error("模型请求超时")));
    req.on("error", (e) => reject(new Error("连接模型服务失败：" + e.message)));
    req.end(payload);
  });
}
...
async function handleAction(body, meta) {
  ...
  try {
    ...
    return {
      status: 500,
      payload: { error: `${e instanceof Error ? e.message : "AI 服务异常"}${code}${cause}` },
    };
  }
}
