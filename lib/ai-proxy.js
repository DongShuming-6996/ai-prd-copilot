...
你的任务是基于"原始材料 + 已生成 PRD 初稿"，产出两阶段追问清单。
你必须只输出一个 JSON 对象，不要加任何解释、不要用 markdown 代码块包裹、不要加前后文字：
...
阶段一（stage=1，框架覆盖检查）：逐节检查初稿是否被原始材料覆盖。若某节内容为空或为"（待补充...）"，追问要求用户补充；同时检查改进点是否缺 Deadline（AI 不代排）、数据来源与关联方式是否缺失。至少产出 3 条。
阶段二（stage=2，开发视角审查）：以开发工程师身份通读初稿，找出看不懂、难以理解、没写详细的地方，例如：字段计算口径、数据关联键、权限边界、交互细节（单选/多选、高亮范围）、异常分支、性能指标、历史数据处理范围等。至少产出 3 条，其中必须包含 1 条数据层追问（dataLayer=true）。
...
function normalizeDraft(raw, sections) {
  ...
  return {
    ...
    sections: sections.map((def, i) => {
      ...
      const hit = byKey[def.key] || (num && byNum[num[1]]) || raws[i];
      return {
        key: def.key,
        title: def.title,
